...
class Ship(object):
		...
		for current_plane in self._planes:
			if current_plane._airborne:
				if self._current_time - current_plane._launch_time < Params.Aircraft.FLIGHT_TIME: # check if flight time has elapsed
					...
					landing_approach_left = Vector2(self._position.x - math.cos(self._angle - math.pi/2), self._position.y - math.sin(self._angle - math.pi/2))
					landing_approach_right = Vector2(self._position.x - math.cos(self._angle + math.pi/2), self._position.y - math.sin(self._angle + math.pi/2))
					...
					distance_from_approach = min(distance_from_left, distance_from_right)
					if distance_from_approach == distance_from_left:
						circle_direction = -math.pi/2
					...
					direction_to_approach_predicted, distance_from_approach_predicted = self.calculate_direction_and_distance(self._position + Vector2(math.cos(self._angle), math.sin(self._angle)) * linearSpeed * dt * distance_from_approach.__mul__(1/Params.Ship.LINEAR_SPEED), current_plane._position)

					# go towards the ship
					if distance_from_approach > 2.0:
						angle_diff = self.calculate_angle_difference(direction_to_ship, distance_from_ship, current_plane._angle)
						self.update_plane_position(current_plane, dt, angle_diff)
					else:
					# start circling the chosen approach point if close enough
						angle_diff = self.calculate_angle_difference(direction_to_approach_predicted, distance_from_approach_predicted, current_plane._angle, circle=True, circle_direction=circle_direction)
						self.update_plane_position(current_plane, dt, angle_diff)
					if self.destination_reached(current_plane._position, self._position) and self._landing_deck_available:
					# plane landed if deck is available
	...
	def calculate_angle_difference(self, direction_to_target, distance_from_target, angle, circle=False, circle_direction=math.pi/2):
		if circle:
			target_angle = math.atan2(direction_to_target.y, direction_to_target.x) + circle_direction # for simple circling just added 90' to an angle 
		else:
